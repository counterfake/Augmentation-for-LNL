...
class cifar_dataset(Dataset):
    def __init__(
        ...
        if self.mode == "test":
            ...
            self.test_data = self.test_data.reshape((self.test_data_size, 3, 32, 32))
            ...
            train_data = train_data.reshape((train_data_size, 3, 32, 32))
            ...
            if os.path.exists(noise_file):
                ...
                noise_label = []
                idx = list(range(train_data_size))
                random.shuffle(idx)
                num_noise = int(self.r * train_data_size)
                noise_idx = idx[:num_noise]
                for i in range(train_data_size):
                    if i in noise_idx:
                        noiselabel = random.randint(0, max(train_label))
